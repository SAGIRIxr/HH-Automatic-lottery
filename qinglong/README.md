...
3. 📊 **定时周期统计简报**：
   - 支持设置每隔 1 小时（或自定义分钟）推送一次时段战报（包含近 1 小时抽数、收支、盈亏、分奖项明细与挂机累计总览）。
4. 📱 **通知卡片精美排版**：
...
- **Ctrl-C / 青龙停止任务**：监听 `SIGINT` 和 `SIGTERM` 信号，在退出前**立即将已抽到的数据安全写入硬盘**并推送最终任务汇总，绝不丢失数据。
